        ...
        <ValidationObserver v-slot="{ handleSubmit }">
            <form class="p-3" @submit.prevent="handleSubmit(signUp)">
                ...
                <ValidationProvider name="E-mail" rules="required|email" v-slot="{ errors }">
                    <div class="form-group mb-4">
                        <input type="email" class="form-control form-round form-primary-purple" placeholder="Email" v-model="form.email" />
                        <span class="form-error" v-if="errors.length > 0">{{
                errors[0]
              }}</span>
                    </div>
                ...
                <ValidationProvider name="Password" rules="required" v-slot="{ errors }">
                    <div class="form-group mb-4">
                        <input type="password" class="form-control form-round form-primary-purple" placeholder="Password" v-model="form.Password" ref="password" />
                        <span class="form-error" v-if="errors.length > 0">{{
                errors[0]
              }}</span>
                    </div>
                ...
                <ValidationProvider name="Password Confirmation" rules="required|password:@Password" v-slot="{ errors }">
                    <div class="form-group mb-4">
                        <input type="password" class="form-control form-round form-primary-purple" placeholder="Type your password again" v-model="form.newPassword" data-vv-as="password" />
                        <span class="form-error" v-if="errors.length > 0">{{
                errors[0]
              }}</span>
                    </div>
                    ...
                    this.$axios
                        ...
                        .then(res => {
                            ...
                            if (res.data.success) {
                                ...
                                this.callError('Ref Monkey Error : ', res.data.message)
                            }
                        })
                        .catch(error => {
